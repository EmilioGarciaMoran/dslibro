...
| click (verb) | clic, hacer clic  |  | clic | hacer clic |
...
| __D__ | | | | |
| dashboard | tablero |un _dashboard_  |  |tablero de control, cuadro de mandos|
| data management | manejo de datos |  | manejo de datos | gestión de datos |
| data set| set de datos |  |  | datos, conjunto de datos |
...
| data wrangling | data wrangling | | procesamiento de datos | procesamiento y conversión de datos |
...
| __F__ | | | | |
| file (verb) | archivar | to file |  | archivar |
| file (noun)| archivo |save the script in a file | Archivo | archivo o fichero |
| for-loop | bucle-for |  | |bucle-for | bucle-for |
| functions | funciones | We will use the functions `mean` and `sd` | funciones | funciones |
...
| __L__ | | | | |
| log transformation | transformación logarítmica | | transformación logarítmica   | transformación logarítmica |
...
| pipe (verb) | canalizar (_pipe_ en inglés) | We can continue to pipe values along |  | enviar a la tubería |
...
| __prebuilt objects__  | objetos predefinidos |   | objetos predefinidos |objetos predefinidos  |
| __Q__ | | | | |
| query | consulta | | consulta |consulta  |
| quote (verb) | entrecomillar  | we need to quote strings |entrecomillar |entrecomillar  |
| __R__ | | | | |
...
| raw data | datos sin procesar  | | datos sin procesar, datos crudos | datos crudos, sin procesar |
| read-in | leer, importar |   | leer (a veces "importar") | leer |
| refresh | actualizar | | actualizar | actualizar |
| regular expression | expresión regular  |  |expresión regular  | expresión regular |
...
| sort| ordenar alfabeticamente o numericamente | sort the rows of this data frame | |ordenar|
| spline | _spline_ |  | un _spline_  | spline ... malla???? |
...
| test set | set de validación |  | set/datos de validación | datos de validación |
...
| match (verb)|  | coincidencia | comparar,confrontar |
| match (noun)|  | coincidencia |comparar  |
| mapping |  | mapear |establecer equivalencias  |
| missing data |  | datos perdidos |datos ausentes|
| missing values || valores faltantes | valores ausentes |  
| parse |   |segmentar o analizar (depende del contexto), parsear  | validación y procesamiento o conversión (muy difícil de traducir |
